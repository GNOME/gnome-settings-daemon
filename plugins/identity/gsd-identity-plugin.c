...
struct GsdIdentityPluginPrivate {
        ...
        GsdIdentityService *identity_service;
        guint               bus_id;
};
...
static void
on_identity_signed_in_for_service (GsdIdentityManager  *manager,
                                   GAsyncResult        *result,
                                   GSimpleAsyncResult  *service_result)
{
        ...
        error = NULL;
        identity = gsd_identity_manager_sign_identity_in_finish (manager,
                                                                 result,
                                                                 &error);

        if (error != NULL) {
                g_debug ("GsdIdentityPlugin: Identity could not be signed in: %s",
                         error->message);
                g_simple_async_result_take_error (service_result, error);

        } else {
                g_debug ("GsdIdentityPlugin: Identity %s signed in",
...
static void
handle_identity_service_sign_in_signal (GsdIdentityService  *service,
                                        const char          *identifier,
                                        GVariant            *details,
                                        GCancellable        *cancellable,
                                        ...
                                        GsdIdentityPlugin   *self)
{
        gsd_identity_manager_sign_identity_in (self->priv->identity_manager,
                                               ...
                                               cancellable,
                                               (GAsyncReadyCallback)
                                               on_identity_signed_in_for_service,
                                               result);
}

static void
on_identity_signed_out_for_service (GsdIdentityManager  *manager,
                                    GAsyncResult        *result,
                                    GSimpleAsyncResult  *service_result)
{
        GError *error;
        ...
        error = NULL;
        gsd_identity_manager_sign_identity_out_finish (manager, result, &error);

        if (error != NULL) {
                g_debug ("GsdIdentityPlugin: Identity could not be signed out: %s",
                         error->message);
                g_simple_async_result_take_error (service_result, error);
...
static void
on_got_identity_for_service (GsdIdentityManager  *manager,
                             GAsyncResult        *result,
                             GSimpleAsyncResult  *service_result)
{
        GError *error;
        ...
        error = NULL;
        identity = gsd_identity_manager_get_identity_finish (manager,
                                                             result,
                                                             &error);
        ...
        gsd_identity_manager_sign_identity_out (manager,
                                                identity,
                                                NULL,
                                                (GAsyncReadyCallback)
...
static void
handle_identity_service_sign_out_signal (GsdIdentityService  *service,
                                         const char          *identifier,
                                         GCancellable        *cancellable,
                                         GSimpleAsyncResult  *result,
                                         GsdIdentityPlugin   *self)
{
        gsd_identity_manager_get_identity (self->priv->identity_manager,
                                           identifier,
                                           cancellable,
                                           (GAsyncReadyCallback)
                                           on_got_identity_for_service,
                                           result);
}
...
on_bus_acquired (GDBusConnection   *connection,
                 ...
                 GsdIdentityPlugin *self)
{
        g_debug ("GsdIdentityPlugin: Connected to session bus");

        if (self->priv->identity_service == NULL) {
                GError *error;

                error = NULL;
                self->priv->identity_service = gsd_identity_service_new (connection,
                                                                         NULL,
                                                                         &error);

                if (self->priv->identity_service == NULL) {
                        g_warning ("Could not create identity service: %s",
                                   error->message);
                }

                g_signal_connect (G_OBJECT (self->priv->identity_service),
                                  "handle-sign-in",
                                  G_CALLBACK (handle_identity_service_sign_in_signal),
                                  self);

                g_signal_connect (G_OBJECT (self->priv->identity_service),
                                  "handle-sign-out",
                                  G_CALLBACK (handle_identity_service_sign_out_signal),
                                  self);
        }
}
                  ...
                  GsdIdentityPlugin *self)
{
        if (g_strcmp0 (name, "org.gnome.Identity") == 0) {
                g_debug ("GsdIdentityPlugin: Acquired name org.gnome.Identity");
        }
}
...
{
        if (g_strcmp0 (name, "org.gnome.Identity") == 0) {
                g_debug ("GsdIdentityPlugin: Lost name org.gnome.Identity");
        }
}
...
impl_deactivate (GnomeSettingsPlugin *plugin)
{
        ...
        g_clear_object (&self->priv->identity_service);
}
