...
static GsdIdentityManager *gsd_kerberos_identity_manager_singleton;
...
static void
get_identity (GsdKerberosIdentityManager *self,
              Operation                  *operation)
{
        GsdIdentity     *identity;
        GError          *error;

        g_debug ("GsdKerberosIdentityManager: get identity %s", operation->identifier);
        identity = g_hash_table_lookup (self->priv->identities,
...
static void
gsd_kerberos_identity_manager_get_identity (GsdIdentityManager     *manager,
                                            const char             *identifier,
                                            GCancellable           *cancellable,
                                            ...
                                            gpointer                user_data)
{
        ...
        result = g_simple_async_result_new (G_OBJECT (self),
                                            callback,
                                            user_data,
                                            gsd_kerberos_identity_manager_get_identity);
        operation = operation_new (self,
                                   cancellable,
                                   OPERATION_TYPE_GET_IDENTITY,
                                   result);
...
static GsdIdentity *
gsd_kerberos_identity_manager_get_identity_finish (GsdIdentityManager  *self,
                                                   GAsyncResult        *result,
                                                   GError             **error)
{
        GsdIdentity *identity;
...
{
        interface->get_identity = gsd_kerberos_identity_manager_get_identity;
        interface->get_identity_finish = gsd_kerberos_identity_manager_get_identity_finish;
                           ...
                           GError                    **error)
{
        ...
        if (error_code != 0) {
                ...
                g_set_error_literal (error,
                                     GSD_IDENTITY_MANAGER_ERROR,
                                     GSD_IDENTITY_MANAGER_ERROR_ACCESSING_CREDENTIALS,
                                     error_message);
                krb5_free_error_message (self->priv->kerberos_context, error_message);
        ...
        if (strcmp (cache_type, "FILE") != 0 &&
            strcmp (cache_type, "DIR") != 0) {
                g_set_error (error,
                             GSD_IDENTITY_MANAGER_ERROR,
                             GSD_IDENTITY_MANAGER_ERROR_UNSUPPORTED_CREDENTIALS,
...
GsdIdentityManager *
gsd_kerberos_identity_manager_new (GCancellable  *cancellable,
                                   GError       **error)
{
        if (gsd_kerberos_identity_manager_singleton == NULL) {
                GObject *object;

                object = g_object_new (GSD_TYPE_KERBEROS_IDENTITY_MANAGER, NULL);

                gsd_kerberos_identity_manager_singleton = GSD_IDENTITY_MANAGER (object);
                g_object_add_weak_pointer (object, (gpointer *) &gsd_kerberos_identity_manager_singleton);
                ...
                g_object_ref (gsd_kerberos_identity_manager_singleton);
        }

        return gsd_kerberos_identity_manager_singleton;
}
