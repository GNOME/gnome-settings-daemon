...
static gboolean gsd_kerberos_identity_is_signed_in (GsdIdentity *identity);
static void set_error_from_krb5_error_code (GsdKerberosIdentity  *self,
                                            GError              **error,
                                            gint                  code,
...
gsd_kerberos_identity_dispose (GObject *object)
{
        ...
        G_OBJECT_CLASS (gsd_kerberos_identity_parent_class)->dispose (object);
...
static void
gsd_kerberos_identity_get_property (GObject    *object,
                                    guint       property_id,
                                    GValue     *value,
        ...
        switch (property_id) {
                ...
                case PROP_IS_SIGNED_IN:
                        g_value_set_boolean (value,
                                             gsd_kerberos_identity_is_signed_in (GSD_IDENTITY (self)));
                        break;
                case PROP_EXPIRATION_TIMESTAMP:
                        G_LOCK (identity_lock);
                        g_value_set_int64 (value,
                                           (gint64) self->priv->expiration_time);
                        G_UNLOCK (identity_lock);
                        break;
                default:
...
gsd_kerberos_identity_class_init (GsdKerberosIdentityClass *klass)
{
        ...
        object_class->get_property = gsd_kerberos_identity_get_property;
...
static char **
get_identifier_components (GsdKerberosIdentity *self)
{
        krb5_principal principal;
        krb5_error_code error_code;
        int size, i;
        ...
        g_ptr_array_add (array,
                         g_strndup (principal->realm.data,
                                    principal->realm.length));

        size = krb5_princ_size (self->priv->kerberos_context,
                                principal);
        ...
        for (i = 0; i < size; i++) {
                ...
                component = krb5_princ_component (self->priv->kerberos_context,
                                                  principal,
                                                  i);
...
static char **
gsd_kerberos_identity_get_identifier_components (GsdIdentity *identity)
{
        GsdKerberosIdentity *self = GSD_KERBEROS_IDENTITY (identity);
...
{
        g_object_notify (G_OBJECT (request->self),
...
static void
queue_notify (GsdKerberosIdentity *self,
              guint               *idle_id,
              const char          *property_name)
{
...
static void
set_expiration_time (GsdKerberosIdentity *self,
                     krb5_timestamp       expiration_time)
{
...
identity_interface_init (GsdIdentityInterface *interface)
{
        ...
        interface->get_identifier_components = gsd_kerberos_identity_get_identifier_components;
                                     ...
                                     GError        **error)
{
        ...
        switch (self->priv->cached_verification_level) {
                ...
                case VERIFICATION_LEVEL_SIGNED_IN:
                        ...
                        queue_notify (self,
                                      &self->priv->is_signed_in_idle_id,
                                      "is-signed-in");
                        return TRUE;
...
static void
update_identifier (GsdKerberosIdentity *self,
...
static void
update_identifier_components (GsdKerberosIdentity *self,
                              GsdKerberosIdentity *new_identity)
{
        char **new_components;
