...
#define GSD_SUBSCRIPTION_MANAGER_GET_PRIVATE(o) (gsd_subscription_manager_get_instance_private (o))
...
G_DEFINE_TYPE_WITH_PRIVATE (GsdSubscriptionManager, gsd_subscription_manager, G_TYPE_OBJECT)
...
{
	GsdSubscriptionManagerPrivate *priv = GSD_SUBSCRIPTION_MANAGER_GET_PRIVATE (manager);
	GVariantBuilder builder;
...
{
	const gchar *argv[] = { "gnome-control-center", "system", "subman", NULL };
	g_debug ("Running gnome-control-center system subman");
...
gsd_subscription_manager_finalize (GObject *object)
{
    ...
    priv = GSD_SUBSCRIPTION_MANAGER_GET_PRIVATE (manager);
	...
	g_clear_object (&priv->notification_expired);
	g_clear_object (&priv->notification_registered);
	g_timer_destroy (priv->timer_last_notified);
