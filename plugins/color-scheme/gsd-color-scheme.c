...
static void
set_scheme (GsdColorScheme *self, gboolean is_dark)
{
    ...
    if (is_dark)
        ...
        new_scheme = G_DESKTOP_COLOR_SCHEME_PREFER_LIGHT;
...
{
    gdouble frac_day;
    gdouble schedule_sunrise = -1.f;
    gdouble schedule_sunset = -1.f;
    g_autoptr(GDateTime) dt_now = gsd_color_scheme_get_date_time_now (self);
    ...
    if (g_settings_get_boolean (self->scheme_settings, "schedule-automatic")) {
        ...
        if (self->cached_sunrise > 0.f && self->cached_sunset > 0.f) {
            schedule_sunrise = self->cached_sunrise;
            schedule_sunset = self->cached_sunset;
        }
    }

    /* Manual */
    if (schedule_sunrise <= 0.f || schedule_sunset <= 0.f) {
        schedule_sunrise = g_settings_get_double (self->scheme_settings,
                                                "schedule-from");
        schedule_sunset = g_settings_get_double (self->scheme_settings,
                                               "schedule-to");
    }
    ...
    g_debug ("fractional day = %.3f, dark = %.3f->%.3f",
             frac_day, schedule_sunrise, schedule_sunset);
