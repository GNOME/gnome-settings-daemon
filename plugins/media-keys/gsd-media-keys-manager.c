...
grab_media_keys (GsdMediaKeysManager *manager)
{
        ...
        g_variant_builder_init (&builder, G_VARIANT_TYPE ("a(sub)"));
        ...
        for (i = 0; i < manager->priv->keys->len; i++) {
                ...
                g_variant_builder_add (&builder, "(sub)", tmp, key->modes, key->ignore_key_repeat);
                g_free (tmp);
        }
		...
		GsdMediaKeysManager *manager)
{
	...
	shell_key_grabber_call_grab_accelerator (manager->priv->key_grabber,
	                                         binding, key->modes, key->ignore_key_repeat,
	                                         manager->priv->grab_cancellable,
...
add_key (GsdMediaKeysManager *manager, guint i)
{
	...
	key->ignore_key_repeat = media_keys[i].ignore_key_repeat;
