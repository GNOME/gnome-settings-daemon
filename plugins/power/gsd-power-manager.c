...
static void
action_suspend_then_hibernate (GsdPowerManager *manager)
{
        if (manager->priv->logind_proxy == NULL) {
                g_warning ("no systemd support");
                return;
                      ...
                      GsdPowerActionType action_type)
{
        ...
        case GSD_POWER_ACTION_HIBERNATE:
                ...
                break;
        case GSD_POWER_ACTION_SUSPEND_THEN_HIBERNATE:
                action_suspend_then_hibernate (manager);
                break;
...
idle_configure (GsdPowerManager *manager)
{
        ...
        if (timeout_sleep != 0) {
                ...
                if (action_type == GSD_POWER_ACTION_LOGOUT ||
                    action_type == GSD_POWER_ACTION_SUSPEND_THEN_HIBERNATE ||
