                                 ...
                                 CcWwanDevice   *device)
{
        ...
        /* Cancellable should be NULL here */
...
object_added_cb (GsdWwanManager *self, GDBusObject *object, GDBusObjectManager *obj_manager)
{
        ...
        /* Unlock the first device to unlock only if no device is being unlocked,
         * We ask to unlock all pending devices instead of the one currently added.
         * If there is only one device (ie, the one which is added) pending, it will
         * make no difference.
         */
        if (self->devices_to_unlock->len > 0 && !self->unlocking_device)
...
gsd_wwan_manager_set_unlock_sim (GsdWwanManager *self, gboolean unlock)
{
        ...
        /*
         * XXX: Should the devices in ‘self->devices’ should be moved to
